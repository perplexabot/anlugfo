...
Example 2:
    ...
    The item with value = 1 occurs in items1 with weight = 1 and in items2 with weight = 3, total
        weight = 1 + 3 = 4.
    The item with value = 2 occurs in items1 with weight = 3 and in items2 with weight = 1, total
        weight = 3 + 1 = 4.
    The item with value = 3 occurs in items1 with weight = 2 and in items2 with weight = 2, total
        weight = 2 + 2 = 4.
...
Example 3:
    ...
    Output: [[1,7],[2,4],[7,1]]
    Explanation:
    The item with value = 1 occurs in items1 with weight = 3 and in items2 with weight = 4, total
        weight = 3 + 4 = 7.
    The item with value = 2 occurs in items1 with weight = 2 and in items2 with weight = 2, total
        weight = 2 + 2 = 4.
...
A:
    one of items is empty:
        who cares
    no equal values:
        concat of two items
    what order to return:
        ascending
...
class Solution:
    def mergeSimilarItems(
        ...
        for v, w in items1 + items2:
            d[v] += w
        return sorted([[k, d[k]] for k in d], key=lambda x: x[0])
...
sol = Solution()
for item1, item2, exp in cases:
    assert (
        got := sol.mergeSimilarItems(item1, item2)
    ) == exp, f"Failed case ({item1}, {item2}) - expecting ({exp}), got ({got})"
