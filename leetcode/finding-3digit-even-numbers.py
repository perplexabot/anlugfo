...
A:
    empty digits -> return []
    no even digits -> return []
    size of digits < 3 -> return []

D:
    get permutations of digits
    if perm is > 99 and even, append to final list
    return final list
"""
...
class Solution:
    def findEvenNumbers(self, digits: List[int]) -> List[int]:
        ...
        for perm in set(permutations(digits, 3)):
            x = int(''.join([str(d) for d in perm]))
            if x > 99 and not x % 2:
...
for (digits, exp) in cases:
    assert (
        got := sol.findEvenNumbers(digits)
    ) == exp, f"Failed case ({digits}) - expecting ({exp}), got ({got})."
