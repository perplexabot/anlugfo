...
Given an integer array nums containing distinct positive integers, find and return
    any number from the array that is neither the minimum nor the maximum value in
    the array, or -1 if there is no such number.
...
Example 3:
    Input: nums = [2,1,3]
    Output: 2
    Explanation: Since 2 is neither the maximum nor the minimum value in nums, it is the
        only valid answer.
...
class Solution:
    def findNonMinOrMax(self, nums: List[int]) -> int:
        i, a = min(nums, default=None), max(nums, default=None)
...
for case in cases:
    assert (got := sol.findNonMinOrMax(case)) in (
        exp := set(case) - set([min(case), max(case)]) if len(set(case)) > 2 else {-1}
