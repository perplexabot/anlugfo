...
You are given a string title consisting of one or more words separated by a single space, where each
    word consists of English letters. Capitalize the string by changing the capitalization of each
    word such that:
...
for (title, exp) in cases:
    assert (
        got := sol.capitalizeTitle(title)
    ) == exp, f"Failed case ({title}) - expecting ({exp}), got ({got})."
