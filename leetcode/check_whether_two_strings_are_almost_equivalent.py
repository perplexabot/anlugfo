...
class Solution:
    def checkAlmostEquivalent(self, word1: str, word2: str) -> bool:
        c0 = Counter(word1)
        c1 = Counter(word2)
        elems = set(word1).union(set(word2))
        for elem in elems:
            if abs(c0[elem] - c1[elem]) > 3:
                return False
        return True
...
cases = [
    ...
    ("a", "a", True),
    ("a", "z", True),
    ("a", "aaaaaaaaaaaaa", False),
]

for w0, w1, exp in cases:
    assert (
        got := sol.checkAlmostEquivalent(w0, w1)
    ) == exp, f"Failed case ({w0}, {w1}) - expecting ({exp}), got ({got})."
