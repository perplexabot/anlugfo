...
A:
    ...
    word of size 1
        return 1
    word of size 2 and ch is second
        reverse all
    word of size 2 and ch is first
        return as is
"""
...
sol = Solution()
for word, ch, exp in cases:
    assert (
        got := sol.reversePrefix(word, ch)
    ) == exp, f"Failed case ({word}, {ch}) - expecting ({exp}), got ({got})."
