"""
You are given a 0-indexed integer array nums. You are also given an integer key, which is present
    in nums.
...
A:
    ...
    array < 2 -> not possible
    key not in array -> always present
    multiple targets -> not possible

D:
    new = [potential_targets]
    Counter(new)
...
sol = Solution()
for nums, key, exp in cases:
    assert (
        got := sol.mostFrequent(nums, key)
    ) == exp, f"Failed case ({nums}, {key}) - expecting ({exp}), got ({got})."
