...
You are given the strings key and message, which represent a cipher key and a secret message,
    ...
    - For example, given key = "happy boy" (actual key would have at least one instance of each
        letter in the alphabet), we have the partial substitution table of ('h' -> 'a',
            'a' -> 'b', 'p' -> 'c', 'y' -> 'd', 'b' -> 'e', 'o' -> 'f').
...
for key, message, exp in cases:
    assert (
        got := sol.decodeMessage(key, message)
    ) == exp, f"Failed case ({key}, {message}) - expecting ({exp}), got ({got})."
