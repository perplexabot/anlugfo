...
D:
    convert to string
    get max by remapping leading digit not equal to 9 to a 9
    get min by remapping leading digit to 0
*/
...
func minMaxDifference(num int) int {
    ...
    for _, char := range s {
        if string(char) != "9" {
            max_replace = string(char)
            break
        }
    }

    min_replace := string([]rune(s)[0])

    min_num, _ := strconv.Atoi(strings.ReplaceAll(s, min_replace, "0"))
    max_num, _ := strconv.Atoi(strings.ReplaceAll(s, max_replace, "9"))
