...
Example 2:
    ...
    Output: "f"
    Explanation: The smallest character that is lexicographically greater than 'c' in letters
        is 'f'.
...
class Solution:
    def nextGreatestLetter(self, letters: List[str], target: str) -> str:
        w = {c: ind for ind, c in enumerate(ascii_lowercase)}
...
sol = Solution()
for letters, target, exp in cases:
    assert (
        got := sol.nextGreatestLetter(letters, target)
    ) == exp, f"Failed case ({letters}, {target}) - expecting ({exp}), got ({got})."
