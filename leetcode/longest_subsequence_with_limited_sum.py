...
Example 2:
    ...
    Output: [0]
    Explanation: The empty subsequence is the only subsequence that has a sum less than or equal to
        1, so answer[0] = 0.
...
sol = Solution()
for nums, queries, exp in cases:
    assert (
        got := sol.answerQueries(nums, queries)
    ) == exp, f"Failed case ({nums}, {queries}) - expecting ({exp}), got ({got})."
