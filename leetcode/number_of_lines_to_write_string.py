...
You are given a string s of lowercase English letters and an array widths denoting how many pixels
    wide each lowercase English letter is. Specifically, widths[0] is the width of 'a', widths[1]
...
D:
    ...
    for each letter in s:
        if width[letter] + curr_size <= 100:
            curr_size += width[letter]
        else:
            line_cnt += 1
            curr_size = width[letter]
    return [line_cnt, curr_size]
"""
...
class Solution:
    def numberOfLines(self, widths: List[int], s: str) -> List[int]:
        ...
        line_cnt = 1
        for letter in s:
            if widths[letter_to_index[letter]] + curr_size <= 100:
                curr_size += widths[letter_to_index[letter]]
            else:
                line_cnt += 1
                curr_size = widths[letter_to_index[letter]]
        return [line_cnt, curr_size]
...
sol = Solution()
for widths, s, exp in cases:
    assert (
        got := sol.numberOfLines(widths, s)
