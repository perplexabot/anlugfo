...
A:
    if len(s) < k -> return s?
    if k = 1 -> not possible
    k = len(s) -> nothing special

D:
    Input: s = "11111222223" = 111, 112, 222, 23, k = 3
        111 -> 3
        112 -> 4
        222 -> 6
        23 -> 5
...
class Solution:
    def digitSum(self, s: str, k: int) -> str:
        ...
        while len(curr) > k:
            new = []
            for chunk in [curr[i : i + k] for i in range(0, len(curr), k)]:
                new.append(str(sum(int(x) for x in chunk)))
            curr = ''.join(new)
        return curr
